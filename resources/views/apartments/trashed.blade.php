                ...
                <div class="card-list d-flex flex-column justify-content-center">
                    @foreach ($apartments as $apartment)
                        @if (Auth::user()->id == $apartment['user_id'])
                                    ...
                                    <div class="col-12 col-md-8">
                                        <div class="card-body pt-5 ps-5">
                                            <a class="title" href="{{ route('apartments.show', $apartment) }}">
                                                <h2 class="card-title mb">{{ $apartment->name }}</h2>
                                            </a>

                                            <p class="card-text"> {{ $apartment->address }}</p>

                                            <div class="apart-data">
                                                <span class="card-text me-3"><strong>Camere da letto:</strong>
                                                    {{ $apartment->n_room }}</span>
                                                <span class="card-text me-3"><strong>Letti:</strong>
                                                    {{ $apartment->n_bed }}</span>
                                                <span class="card-text"><strong>Bagni:</strong>
                                                    {{ $apartment->n_bath }}</span>
                                            </div>


                                            <div class="buttons d-flex justify-content-end align-items-center">
                                                <button type="button" class="custom-btn custom-btn-yellow me-3"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#restoreModal{{ $apartment->id }}">
                                                    Ripristina
                                                </button>
                                                <button type="button" class="custom-btn custom-btn-red"
                                                    data-bs-toggle="modal"
                            ...
                            {{-- Modale Ripristino --}}
                            <div class="modal fade" id="restoreModal{{ $apartment->id }}" tabindex="-1"
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <p>Sei sicuro di voler ripristinare <strong>"{{ $apartment->name }}"</strong>?
                                            </p>
                                            <div class="alert alert-warning warning">
                                                <h2><i class="fa-solid fa-triangle-exclamation"></i> Attenzione</h2>
                                                <p>Il ripristino dell'appartamento sarà immediato e risulterà
                                                    automaticamente visibile.</p>
                                            </div>
                                        </div>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="alert alert-danger">
                                                <h2><i class="fa-solid fa-triangle-exclamation"></i> Attenzione</h2>
                                                <p>L'eliminazione di questo appartamento sarà definitiva, l'azione non è
                                                    reversibile.</p>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-outline-dark"
                                                data-bs-dismiss="modal">Annulla</button>
                                            <form action="{{ route('apartments.forceDestroy', $apartment->id) }}"
                                                method="POST">
                                                @method('delete')
                                                @csrf
